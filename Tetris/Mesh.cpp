...
void Mesh::CreateMesh(unsigned int* indices, unsigned int numOfIndices, float* vertices, unsigned int numOfVertices)
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(m_Indices[0]) * m_NumOfIndices, m_Indices, GL_STATIC_DRAW);

	// Generate and bind vertex buffer object
	glGenBuffers(1, &m_VBO);
	glBindBuffer(GL_ARRAY_BUFFER, m_VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(m_Vertices[0]) * m_NumOfVertices, m_Vertices, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(m_Vertices[0]) * 2, 0);
